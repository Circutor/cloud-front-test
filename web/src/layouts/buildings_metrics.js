...
import { Layout, Menu, DatePicker, Typography, Tabs } from 'antd';
  ...
  useEffect(() => {
    ...
    GetBuildingMetrics(
      ...
      for (let i = 0; i < metricsData.length - 1; i++) {
        const row = [
          new Date(Date.parse(metricsData[i].timestamp)),
          metricsData[i].value,
        ];
        tmpData.push(row);
      }
    ...
    <Layout style={{ minHeight: '100vh' }}>
      <Header>
        ...
        <Menu theme='dark' mode='horizontal' defaultSelectedKeys={['metrics']}>
          <Menu.Item key='all' onClick={goToBuildings}>
            All
          </Menu.Item>
          <Menu.Item key='bookmarks' onClick={goToBookmarks}>
            Bookmarks
          </Menu.Item>
        </Menu>
        <Button
